...
public class Column extends SudokuGroup {
    ...
    public Column(int index, ArrayList<SudokuField> allFields) {
    ...
    public ArrayList<SudokuField> findRelatedFields(ArrayList<SudokuField> allFields) {
        ArrayList<SudokuField> relatedFields = allFields.stream()
                .filter(field -> field.isInColumn(this.getIndex()))
                .collect(Collectors.toCollection(ArrayList::new));

        if(relatedFields.size() != 9){
            throw new RuntimeException("Error in Field-Initialisation Column " + this.getIndex());
        }
